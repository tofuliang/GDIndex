    ...
    <v-container fluid>
        <portal to="navbar">
            <v-toolbar-items>
                <template v-for="seg in pathSegments">
                    ...
                    <v-btn text class="text-none" :key="seg.path + '-btn'" @click="goPath(seg.path)">{{
                        seg.name
                    }}</v-btn>
                </template>
            </v-toolbar-items>
        ...
        <v-row justify="center">
            <v-col md="8" lg="6">
                ...
                <v-card class="mx-auto" min-height="400px" tile :loading="loading" v-else>
                    <v-list-item
                        v-for="item in list"
                        :key="item.id"
                    ...
                    >
                        ...
                        <v-list-item-content class="py-2">
                            ...
                            <v-list-item-subtitle v-if="!item.isFolder" v-text="item.fileSize"></v-list-item-subtitle>
                        </v-list-item-content>
                        <v-list-item-action>
                            <v-btn
                                icon
...
export default {
    ...
    computed: {
        ...
        uploadUrl() {
            ...
            u.searchParams.set('rootId', this.$route.query.rootId || window.props.default_root_id);
            return u.href;
        }
    },
    methods: {
        ...
        async renderPath(path, rootId) {
            ...
            this.list = files.map(f => {
                ...
                const escapedFilename = encodeURIComponent(util.replaceSpecialCharacter(f.name));
                const resourcePath = nodeUrl.resolve(path, escapedFilename) + (isFolder ? '/' : '');
                const o = {
                    fileName: f.name,
                    modifiedTime: format(new Date(f.modifiedTime), 'yyyy/MM/dd HH:mm:ss'),
                    isFolder,
                    isGoogleFile,
                    mimeType: f.mimeType,
        ...
        handlePath(path, query) {
            if (path.substr(-1) === '/') {
                ...
                if (query.rootId && query.rootId !== window.props.default_root_id) {
...
.theme--light.v-data-table tbody .fake-tr:hover:not(.v-data-table__expanded__content) {
    background: #eeeeee;
}
.theme--light.v-data-table tbody .fake-tr:not(:last-child) td:not(.v-data-table__mobile-row) {
